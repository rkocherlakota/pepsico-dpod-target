...
class Trainer:
    def __init__(self, exp: Exp, args):
        ...
        self.scaler = torch.amp.GradScaler(enabled=args.fp16 and torch.cuda.is_available())
    ...
    def train_one_iter(self):
        ...
        if isinstance(batch, (tuple, list)) and len(batch) >= 2:
            inps, targets = batch[:2]
        else:
            raise ValueError(f"Unexpected batch format returned from prefetcher: {batch}")
        # print(f"[DEBUG] Prefetcher returned batch of type {type(batch)} with length {len(batch)}")
